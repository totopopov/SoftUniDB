...
public class AddToShoppingCartCommand extends BaseCommand {
    ...
    public AddToShoppingCartCommand(DataHandlerAble dataHandlerAble, Parameters parameters) {
...
//        UserDtoCartInfo currentUser = super.getDataHandlerAble().getUserService().findById(userId);
        ...
        currentUser.getGamesInCart().addAll(byTittleForCart);

        Boolean sucess = super.getDataHandlerAble().getUserService().addToUserCarts(currentUser);

        return sucess?String.format("%s added to cart",title):"Something went wrong";
    }
}
